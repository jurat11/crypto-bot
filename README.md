...
11. **Orders & fills:** every demo and testnet order: quantity, average fill price, $ value, fee, and slippage against the mid price. For testnet fills, the line under the slippage shows what the simulator predicted from the same testnet order book, so the real fill can be compared with the simulation. Testnet prices are labelled TESTNET, because testnet prices can differ from the real market.
