...
def check(cfg, state, equity, data_age_hours, orders):
    r = cfg["risk"]
    problems = []
    if os.path.exists(KILL_SWITCH):
        problems.append("kill switch file STOP exists")
    if data_age_hours > r["max_data_age_hours"]:
        problems.append(f"price data is {data_age_hours:.0f}h old")
    ...
    if last and 1 - equity / last >= r["max_daily_loss_pct"]:
        problems.append(f"lost {1 - equity / last:.0%} since last run (limit {r['max_daily_loss_pct']:.0%})")
